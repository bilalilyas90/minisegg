    ...
    def readFile(self, fileName, trainStg=False):
        ...
        with open(osp.join(self.data_dir, fileName), 'r') as text_file:
            for line in text_file:
                ...
                line_arr = line.split()
                print(line_arr)
                # img_file = osp.join(self.data_dir, line_arr[0].strip()+'.png')
                # label_file = osp.join(self.data_dir, line_arr[1].strip()+'.png')
                
                img_file = self.data_dir+"/sunny/JPEGImages/"+ line_arr[0].strip()+'.png'
                label_file = self.data_dir+"/sunny/SegmentationClassAug/"+ line_arr[1].strip()+'.png'
